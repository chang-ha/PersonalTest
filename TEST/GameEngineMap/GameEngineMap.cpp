...
// 장점
// 1. 맵은 자료가 많을 떄(자료가 천개 이하면 vector가 더 빠름...;;) 탐색을 용이하게 하기 위해서 사용한다.
// 2. 맵은 사용성이 좋다. (vector는 특정 자료를 찾으려면 내가 for문 돌려야함, map은 내부에 함수로 구현되어 있음)


// 수업에서 만든 map은 이진탐색 트리(바이너리 서치 트리)구조인데
// 이 구조의 문제점은 편향트리가 되는것을 해결할 수 없다는 것 (편향트리 : 한쪽(왼쪽자식 or 오른쪽 자식)으로 자료가 치우친 것)
// 편향트리가 되면 자료구조의 효율성이 떨어짐
// 그래서 편향트리를 해결하는 여러 알고리즘이 나왔는데
// std::map에서는 레드블랙이진트리 라는 알고리즘을 통해 편향트리를 해결하였음
